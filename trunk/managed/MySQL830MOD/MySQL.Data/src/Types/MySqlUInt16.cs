...
namespace MySql.Data.Types
{
  internal struct MySqlUInt16 : IMySqlValue
  {
    ...
    async Task IMySqlValue.WriteValueAsync(MySqlPacket packet, bool binary, object val, int length, bool execAsync)
    {
      ...
      if (binary)
        await packet.WriteIntegerAsync(v, 2, execAsync).ConfigureAwait(false);
      else
    ...
    async Task<IMySqlValue> IMySqlValue.ReadValueAsync(MySqlPacket packet, long length, bool nullVal, bool execAsync)
    {
      ...
      if (length == -1)
        ...
        return new MySqlUInt16(UInt16.Parse(await packet.ReadStringAsync(length, execAsync).ConfigureAwait(false), CultureInfo.InvariantCulture));
    }
